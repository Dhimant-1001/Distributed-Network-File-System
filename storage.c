...
int get_local_ip(char *ip_buffer, size_t buffer_size) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (fp != NULL) {
            if (fgets(ip_buffer, buffer_size, fp) != NULL) {
                ...
                pclose(fp);
                if (strlen(ip_buffer) > 0 && strcmp(ip_buffer, "127.0.0.1") != 0) {
                    return 0; // Success
                }
            }
            pclose(fp);
    ...
    printf("Warning: Could not determine local IP, using localhost\n");
    return -1;
}
...
int main(int argc, char *argv[])
{
    ...
    // Get the local IP address using cross-platform method
    char storage_ip[BUFFER_SIZE];
    if (get_local_ip(storage_ip, sizeof(storage_ip)) < 0) {
        fprintf(stderr, "Warning: Using fallback IP address\n");
    }
